...
if (!empty($HTTP_POST_VARS["FORM"])){
		...
		if ($receber_0 == 1){  // Cartao,Cheque,Deposito,Boleto
			...
			if( empty($erros) ){
					...
					$consulta  = "UPDATE tbl_carrinho SET ID_PEDIDO_CLIENTE = '$nov_id_cli' WHERE `sessao` = '$send_sesao' ";
					
					@mysql_query($consulta, $link);
					
					
					// Consultando Pedidos do Cliente
					$consulta1  = "select * from tbl_carrinho WHERE ID_PEDIDO_CLIENTE = '$nov_id_cli'";
					
					$resultado1 = @mysql_query($consulta1);
					
					
					// Consultando Nome do Cliente
					$consulta2  = "select * from pedidos_cliente WHERE ID_PEDIDO_CLIENTE = '$nov_id_cli'";
					...
					?>
									...
									<?
										...
										while ($linha = @mysql_fetch_array($resultado1))
										{
											...
											<center><b>ATENÇÃO para concretizar sua compra clique</b><br />
								            <center><b>no botao a baixo e preencha as informacoes</b><br/></center>
								            <center><b>solicitadas corretamente<b><br /><br />
								
											
											<input type="image" src="graphics/btnConcluirBR.jpg" name="submit" alt="Pague com PagSeguro - é rápido, grátis e seguro!"/><br /><br /><br />
							    ...
							    $phpmail->IsHTML(true);
							    $phpmail->From     = "edson@sindificios.com.br";
							    $phpmail->FromName = "Portal da Troca";
							
							    $phpmail->AddAddress("sysmp1@gmail.com");
							    $phpmail->AddAddress($_POST["TMAIL"]);
								...
								// Consultando Pedidos do Cliente
								$consulta1z  = "select * from tbl_carrinho WHERE ID_PEDIDO_CLIENTE = '$nov_id_cli'";
								
								$resultado1z = @mysql_query($consulta1z);

								// Consulta Cliente
								$consulta11  = "SELECT * FROM pedidos_cliente Where ID_PEDIDO_CLIENTE = '$nov_id_cli'";
							    ...
							    $phpmail->Subject = "Envio de email do Portal iSysMP.com";
							    $phpmail->Body .= "Nome: ".$_POST["TNOME"]."<br />";
							    $phpmail->Body .= "E-mail: ".$_POST["TMAIL"]."<br />";
							    $phpmail->Body .= "CPF: " . $cpf . "<br />";
							    $phpmail->Body .= "Assunto: Compra de Produtos Portal iSysMp".$_POST['assunto']."<br />";
							    ...
							    if($send){
									...
									$consulta_up1  = "UPDATE tbl_carrinho SET `sessao` = ' ' WHERE ID_PEDIDO_CLIENTE = '$nov_id_cli'  ";
							    	...
							    	echo "<font face=Tahoma><font size=4 color='#FF0000'><b>A T E N Ç Ã O</b>";
									...
									echo "<center><b>um erro de Servidor Reconecte, e tente de novo.</b></center>";
			     ...
			     echo "<br><br><br><br><center><font color='#FF0000' face='Arial' size='4'><b>ATENÇÃO</b></font></center><br>";
			     echo "<center><b>" . $erros . "</b></center><br><br>";
			     
			     echo "<center><a href='carrinho_fim.php'><img src='graphics/voltar_.png' border='0'/></a></center>";
			}
						...
						$i          = 0;
							    ...
							    if($send){
									...
									echo "<center><b>com o vendedor ou com o Portal iSysMP.com</b></center>";
									echo "<center><b>pelo telefone ou acesse <a href=../fale/layout.php>Fale com o Portal</a></b></center>";
									echo "<center><b>Obrigado</b></center></body>";
									$faz_x1 = 1;
